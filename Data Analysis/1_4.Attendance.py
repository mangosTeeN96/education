...
# 通过年月对于数据集重新排序，然后学期栏缺失值直接引用上一个即可
...
def create_early_json():
    json_data = {"row": [statistic_year_early(), statistic_term_early(), statistic_month_early(), statistic_day_early()],
...
def statistic_peak_hour():
    ...
    # 生成time_reg_all：00～09，time：00：00～09：00
    for m in range(10):
        time_reg = '0' + str(m)
    ...
    # 生成time_reg_all：00～23，time：00：00～23：00
    for i in range(10, 24):
        time_reg_all.append(str(i))
...
def statistic_peak_minute2():
    ...
    for i in range(60):
        for j in range(data_peak_out2.shape[0]):
            if data_peak_out2['minute'].iloc[j] == time_reg_all[i + 60]:
                count[i + 60] += 1
