...
filepath_StudentsInfo_processed = '../education_data/more/Student/1.StudentInfo.csv'
...
filepath_StudentsConsumption_Processed = '../education_data/more/Student/5.Consumption.csv'
...
def statistic_teachers_info(studentID):
    ...
    teacherName = ['NaN'] * 12
    studentInfo = data_StudentInfo[data_StudentInfo['bf_StudentID'] == studentID]
    if studentInfo.shape[0] == 0:
        print('没有该学生的信息，请重新输入！')
    else:
        classID = studentInfo['cla_id'].iloc[0]
        teacherInfo = data_TeacherInfo[data_TeacherInfo['cla_id'] == classID]
...
def data_preprocess_studentInfo():
    ...
    # 保存
    data_StudentInfo.to_csv('../education_data/more/Student/1.StudentInfo.csv', encoding='utf_8_sig')
    print('文件保存完成！')
...
def statistic_class_studentsNum():
    ...
    data_studentsInfo.insert(0, 'count', 1)
...
def statistic_students_attendance():
    ...
    # 首先通过groupby统计考勤数据集中有多少组学生的数据
    data_attendance['count'] = 1
    data_attendance_groupby = data_attendance.groupby(['bf_studentID']).count().reset_index().sort_values(by='count', axis=0, ascending=False)
    # 统计得到的数据有3058条

    # 对数据进行筛选，选取前n（300）个学生的数据
    data_attendance_groupby = data_attendance_groupby.iloc[0: 300]
    # 验证这300个数据是否有学生信息
    studentID_Array = []
    count = 0
    # 把data_attendance_groupby['bf_studentID']转换为数组studentID_Array
    for i in range(data_attendance_groupby.shape[0]):
        studentID_Array.append(data_attendance_groupby['bf_studentID'].iloc[i])

    # 循环嵌套查找这前n个考勤数据集中的学生ID，在学生信息数据集中是否也有
    for i in range(len(studentID_Array)):
        for j in range(data_studentInfo.shape[0]):
            if studentID_Array[i] == data_studentInfo['bf_StudentID'].iloc[j]:
                ...
                count += 1
    # 考勤中的数据存在的，在studentinfo中只有76组（76/300、33/100）
    print(count)
    print('##########################')
    print(data_attendance_groupby.iloc[99])
    print('##########################')
    print(data_attendance_groupby.shape)
...
def statistic_student_attendance(studentID):
    ...
    student_attendance = data_attendance[data_attendance['bf_studentID'] == studentID]
    # 首先统计该学生的考勤记录
    # 统计该学生的迟到、早退、校服校徽问题
    ...
    for i in range(len(late_taskName)):
        student_attendance_late = student_attendance[student_attendance['control_task_order_id'] == late_taskName[i]]
        student_problems_num[0] = student_attendance_late.shape[0]
    for i in range(len(early_taskName)):
        student_attendance_early = student_attendance[student_attendance['control_task_order_id'] == early_taskName[i]]
        student_problems_num[1] = student_attendance_early.shape[0]
    for i in range(len(uniform_taskName)):
        student_attendance_uniform = student_attendance[student_attendance['control_task_order_id'] == uniform_taskName[i]]
    ...
    class_attendance = data_attendance[data_attendance['bf_classid'] == student_classid]
    print('班级考勤数据量为', class_attendance.shape[0])
    for i in range(len(late_taskName)):
        class_attendance_late = class_attendance[class_attendance['control_task_order_id'] == late_taskName[i]]
        class_problems_num[0] = class_attendance_late.shape[0]
    for i in range(len(early_taskName)):
        class_attendance_early = class_attendance[class_attendance['control_task_order_id'] == early_taskName[i]]
        class_problems_num[1] = class_attendance_early.shape[0]
    for i in range(len(uniform_taskName)):
        class_attendance_uniform = class_attendance[class_attendance['control_task_order_id'] == uniform_taskName[i]]
    ...
    student_attendance_hour_6 = student_attendance[student_attendance['hour'] == '06']
    ...
    class_attendance_hour_6 = class_attendance[class_attendance['hour'] == '06']
    ...
    school_attendance_hour_6 = data_attendance[data_attendance['hour'] == '06']
...
def create_confidence_area(rank_array):
    ...
    for i in range(len(rank_array)):
        # 判断上升趋势或者下降趋势
        # i范围为[0~len-1]，因为要判断上升下降趋势，i-1时要>0,所以i=0单独讨论（else）
        if i > 0:
            if rank_array[i] - rank_array[i - 1] > 0:
                # 排名变差，置信区间变小
                # rank_up_array[i]=rank_array[i]-rank_down_array[i]+rank_up=rank_array[i]-rank_array[i]+rank_down+rank_up
                # = rank_down+rank_up
...
def statistic_student_score(studentID):
    ...
    # 得到这个学生所在的班级名称、班级人数
    class_name = data_student_info[data_student_info['bf_StudentID'] == studentID]['cla_Name'].iloc[0]
    classmates_num = data_student_info[data_student_info['cla_Name'] == class_name].shape[0]
    ...
    # 筛选这个学生的成绩
    student_score = data_score[data_score['mes_StudentID'] == studentID]
    ...
    for i in range(score_groupby_examnum.shape[0]):
        exam_score = student_score[student_score['exam_number'] == score_groupby_examnum['exam_number'].iloc[i]]
        # print(exam_score)
        mes_sum = 0
        mes_score_sum = 0
    ...
    for i in range(len(subname)):
        student_sub_score = student_score[student_score['mes_sub_name'] == subname[i]]
        # 学科排名数组、学科名字数组
        sub_rank_array = []
        sub_examname_array = []
        ...
        # 预测排名1
        # sum是sub_rank_array[i-1]、[i-2]、[i-3]的和，预测排名为前三次排名的均值加一个随机数[-3,3]
        sum = 0
        for j in range(3):
            sum += sub_rank_array[len(sub_rank_array) - j - 1]
        average_rank_predict = int(sum / 3) + random.randint(-3, 3)

        # 将排名数组的排名数据和预测的排名数据一起存入sub_rank_predict_array
