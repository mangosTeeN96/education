...
data_sample = pd.read_csv('../education_data/more/5.chengji_1.csv')

data_student_info = pd.read_csv('../education_data/2_student_info.csv')

data_merge_byStdId = pd.read_csv('../education_data/more/5.chengji_2_claID.csv')
...
##############################################################################
# Step2：去除其他的数据，观察2017学年度第一学期期末两次考试的数据
...
def find_2017_finalExam():
    ...
    data_2017_finalExam.to_csv('../education_data/more/5.chengji_1.csv', encoding='utf_8_sig')
...
def merge_student_id():
    ...
    data = data_merge_byStdID.drop(['bf_Name', 'mes_TestID', 'exam_number', 'bf_sex', 'bf_nation', 'bf_BornDate',
                                    'bf_NativePlace', 'Bf_ResidenceType', 'bf_policy', 'cla_term', 'bf_zhusu',
                                    'bf_leaveSchool', 'bf_qinshihao'], axis=1)
    print(data)
    data.to_csv('../education_data/more/5.chengji_2_claID.csv', encoding='utf_8_sig')
