...
def statistic_sub_score_connection(studentID):
    ...
    for i in range(len(subname)):
        ...
        # 如果该科目数据量>10，取前10条数据；<10，用[0]位数据补齐10条数据
    ...
    # print(rank_array_new)
    # print(examname_array_new)
    return rank_array_new
    # 返回的是一位同学的九门考试成绩，每门有十次考试的成绩，数组套数组
...
def extract_connection_data():
    ...
    for i in range(10):
        ...
        connection_data_1.extend(rank_array_c[0])  # 10个学生的10次语文考试（extend，变为一个100个元素的数组）
        connection_data_2.extend(rank_array_c[1])  # 10个学生的10次数学考试
        connection_data_3.extend(rank_array_c[2])  # 10个学生的10次英语考试
        connection_data_4.extend(rank_array_c[3])  # 10个学生的10次物理考试
        connection_data_5.extend(rank_array_c[4])  # 10个学生的10次化学考试
        connection_data_6.extend(rank_array_c[5])  # 10个学生的10次生物考试
        connection_data_7.extend(rank_array_c[6])  # 10个学生的10次政治考试
        connection_data_8.extend(rank_array_c[7])  # 10个学生的10次历史考试
        connection_data_9.extend(rank_array_c[8])  # 10个学生的10次地理考试

    # 合并为一个大列表
    # connection_data_j 为按学生顺序的10个学生的10次j考试（一个学生的10次j考试、一个学生的10次j考试、...）
    # 合并后为 一个同学的某次考试的各科成绩:[0,...,0] 10次数据都输入后:[0,...,0],...,[0,...,0] 下一个同学。
    # 所以合并后为一个9*100的矩阵
    student_sub_score = []
    for i in range(100):
