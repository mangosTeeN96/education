...
# kaoqin_type = pd.read_csv("../education_data/4_kaoqintype.csv")
chengji = pd.read_csv("../education_data/5_chengji.csv")
csp = pd.read_csv("../education_data/7_consumption.csv")
# stu_index = pd.read_csv("../education_data/stu_index.csv")
...
def xiaofei(class_id):
    ...
    csp_stuid = pd.merge(csp, a[['bf_StudentID']], how='right')
    csp_stuid.dropna(axis=0, how='any', inplace=True)
    csp_stuid['MonDeal'] = csp_stuid['MonDeal'].apply(lambda s: -float(s))
    csp_stuid['DealTime'] = csp_stuid['DealTime'].apply(formatDatetime)

    c1 = csp_stuid[['DealTime', 'MonDeal']]
    csp_day_total = c1.groupby('DealTime').sum().reset_index()  # 每天消费总额

    c2 = csp_stuid[['DealTime', 'bf_StudentID']]
    ...
    csp_day_num = c2.groupby('DealTime').sum().reset_index()  # 每天消费人数

    csp_day = pd.merge(csp_day_total, csp_day_num, how='outer')  # 两表合并
    csp_day = csp_day[csp_day['csp_num'] >= 20]

    # 获得max、min、mean
    c4 = csp_stuid[['DealTime', 'MonDeal', 'bf_StudentID']]
    c4 = c4.groupby(['DealTime', 'bf_StudentID']).sum().reset_index()
    c4 = c4[['DealTime', 'MonDeal']]
    c4_max = c4.groupby('DealTime').max().reset_index().rename(index=str, columns={'MonDeal': 'max'})
    c4_min = c4.groupby('DealTime').min().reset_index().rename(index=str, columns={'MonDeal': 'min'})
    c4_mean = c4.groupby('DealTime').mean().reset_index().rename(index=str, columns={'MonDeal': 'mean'})

    # 合并
    csp_res = pd.merge(csp_day, c4_max, how='left')
    csp_res = pd.merge(csp_res, c4_min, how='left')
    csp_res = pd.merge(csp_res, c4_mean, how='left')
    csp_res = csp_res.set_index('DealTime')
    csp_res.rename(index=str, columns={'MonDeal': 'total', 'csp_num': 'peo_num'}, inplace=True)
    csp_res = csp_res[
        (csp_res['total'] > 200) &
        (csp_res['total'] < 1500)]
    return csp_res
...
##################################################################################################
# 考勤
kaoqin_one_class = kaoqin[kaoqin['bf_classid'] == class_id]
kq_1 = kaoqin_one_class[['DataDateTime', 'controler_name', 'bf_studentID']]
kq_1['DataDateTime'] = kq_1['DataDateTime'].apply(formatDatetime)
kq_1['controler_name'] = kq_1['controler_name'].apply(lambda s: s[:2])
kq_1.insert(3, 'peo_num', 1)
d2 = kq_1.groupby(['DataDateTime', 'controler_name']).sum().reset_index().sort_values(by=['DataDateTime'])
print(d2)
# obj = {}
